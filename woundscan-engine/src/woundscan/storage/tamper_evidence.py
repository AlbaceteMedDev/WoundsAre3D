...
def append_to_chain(payload: dict, sequence: int, previous_hash: str) -> HashChainEntry:
    """Build a new chain entry from a payload."""
    payload_json = json.dumps(payload, sort_keys=True)
    self_hash = hashlib.sha256((previous_hash + payload_json).encode()).hexdigest()
...
def verify_chain(entries: Iterable[HashChainEntry]) -> tuple[bool, int]:
    ...
    for expected_sequence, entry in enumerate(entries):
        ...
        if entry.previous_hash != prev:
            return False, last_valid
        expected = hashlib.sha256((entry.previous_hash + entry.payload_json).encode()).hexdigest()
